...
function saveDatabase() {
  if (db) {
    ...
    const dbPath = path.join(electron.app.getPath("userData"), "data", "labflow.db");
...
electron.ipcMain.handle("get-app-path", () => electron.app.getPath("userData"));
electron.ipcMain.handle("db:getTubes", async () => {
  return queryAll("SELECT * FROM tubes WHERE status != ? ORDER BY created_at DESC", ["discarded"]);
});
electron.ipcMain.handle("db:getTube", async (_, id) => {
  return queryOne("SELECT * FROM tubes WHERE id = ?", [id]);
});
electron.ipcMain.handle("db:addTube", async (_, tube) => {
  db.run(
...
electron.ipcMain.handle("db:updateTube", async (_, tube) => {
  db.run(
...
electron.ipcMain.handle("db:deleteTube", async (_, id) => {
  db.run("UPDATE tubes SET status = ? WHERE id = ?", ["discarded", id]);
  saveDatabase();
  return { success: true };
});
electron.ipcMain.handle("db:getExperiments", async () => {
  return queryAll("SELECT * FROM experiments ORDER BY created_at DESC");
});
electron.ipcMain.handle("db:getExperiment", async (_, id) => {
  return queryOne("SELECT * FROM experiments WHERE id = ?", [id]);
});
electron.ipcMain.handle("db:saveExperiment", async (_, exp) => {
  ...
  if (existing) {
    db.run(
      ...
      [
        ...
        exp.status,
        JSON.stringify(exp.tubes),
        JSON.stringify(exp.tube_positions || []),
        JSON.stringify(exp.connections),
        exp.warehouse_snapshot || null,
        exp.initial_state_tubes || null,
        exp.end_state_tubes || null,
        exp.id
      ]
    );
  } else {
